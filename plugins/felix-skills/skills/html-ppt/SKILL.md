...
3. **用口语，口播稿，不用书面语** — "因此"→"所以"，"该方案"→"这个方案"
...
### 📝 逐字稿写在 `notes.js`（推荐，可单独编辑）

**问题**：逐字稿写在 `index.html` 的 `<aside class="notes">` 里，改起来要在 HTML 标签之间跳来跳去，不好用编辑器批量编辑。

**方案**：把逐字稿抽到 deck 目录下的 `notes.js` 里，用一个纯数组，用户只改这个文件就能改逐字稿。**双击 `index.html` 直接跑，不需要起 http server**。

**用法**：

1. 在 deck 目录建一个 `notes.js`：
   ```js
   // notes.js — 每个元素对应一张 slide 的逐字稿（按顺序）
   ...
   *重点*：右边这个柱状图是……`,
   ...
   ```
2. 在 `index.html` 里，**在 `runtime.js` 之前**加载它：
   ```html
   <script src="notes.js"></script>
   <script src="../../../assets/runtime.js"></script>
   ```
3. 保存 `notes.js` → 刷新 audience 窗口（或重新按 `S`）→ 演讲者视图的 SCRIPT 卡片显示新逐字稿。

**支持的 mini-markdown**（在 `notes.js` 里可以用）：
- `**加粗**` → **加粗**
- `*斜体*` → *斜体*
- `` `代码` `` → `代码`
- 空行 → 分段（每段一个 `<p>`）
...
**优先级**（`runtime.js` 内部）：
1. `window.__PPT_NOTES__[i]` 有内容 → 用它
2. 否则读 slide 里的 `<aside class="notes">` → 用它（**完全向下兼容**，老 deck 不用改）
3. 都没有 → 空

### 🔁 `file://` 下双窗口同步

`BroadcastChannel` 在 `file://` 协议下跨窗口失效。runtime.js 用 `localStorage` `storage` 事件做兜底，audience/presenter 双向翻页 + 主题切换在 `file://` 下也能同步。无需任何配置。
...
- **Supply notes.** Wrap speaker notes in `<div class="notes">…</div>` inside
  each slide, **or** put them in an external `notes.js` file (see the 📝 section
  above — better for long 逐字稿, single-file editing). Press S to open the
  overlay.
...
- [references/drawio-integration.md](references/drawio-integration.md) — **draw.io 架构图集成进深色 slide（画复杂架构图/拓扑图/时序图时加载）。** 复杂图先起 subagent 用 drawio skill 画 SVG，再按此文档做深色化 + `<img>` 集成。
