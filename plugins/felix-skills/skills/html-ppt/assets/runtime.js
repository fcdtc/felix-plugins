...
(function () {
  ...
  /* ========== Mini markdown renderer for external notes ==========
   * Supports: **bold**, *italic*, `code`, blank-line paragraphs, line breaks.
   * Input: string. Output: HTML string (escaped, safe from injection).
   */
  function renderMiniMd(s) {
    if (s == null) return '';
    // escape first
    let out = String(s)
      .replace(/&/g, '&amp;')
      .replace(/</g, '&lt;')
      .replace(/>/g, '&gt;');
    // inline: **bold**, *italic*, `code` — order matters (bold before italic)
    out = out.replace(/\*\*([^*\n]+)\*\*/g, '<strong>$1</strong>');
    out = out.replace(/\*([^*\n]+)\*/g, '<em>$1</em>');
    out = out.replace(/`([^`\n]+)`/g, '<code>$1</code>');
  ...
  ready(function () {
    ...
    /* ===== Resolve speaker notes for slide i =====
     * Priority:
     *   1. window.__PPT_NOTES__[i]  — external notes.js (markdown, rendered)
     *   2. <aside class="notes"> inside slide DOM (raw HTML, back-compat)
     *   3. empty string
     */
    function resolveNotes(slide, i) {
      const ext = (typeof window !== 'undefined') ? window.__PPT_NOTES__ : null;
      if (ext && ext[i] != null && ext[i] !== '') {
        return renderMiniMd(ext[i]);
      }
      const note = slide.querySelector('.notes, aside.notes, .speaker-notes');
    ...
    /* ===== file:// fallback sync via localStorage 'storage' event =====
     * BroadcastChannel doesn't work across file:// windows (different origins).
     * localStorage 'storage' event fires cross-window ONLY on the OTHER window,
     * so it's a natural fit for audience ↔ presenter sync when BC fails.
     */
    const LS_SYNC_KEY = 'html-ppt-sync:' + location.pathname;
    function lsBroadcast(msg) {
      try {
        localStorage.setItem(LS_SYNC_KEY, JSON.stringify({ ...msg, _t: Date.now() }));
      } catch(e) {}
    }
    function lsSubscribe(handler) {
    ...
    function go(n, fromRemote){
      ...
      notes.innerHTML = resolveNotes(slides[n], n);
    ...
    function openPresenterWindow() {
      ...
      // Collect slide titles + notes (HTML strings). Uses external notes.js
      // via window.__PPT_NOTES__ if present, else falls back to inline DOM.
      const slideMeta = slides.map((s, i) => {
        return {
          title: s.getAttribute('data-title') ||
            (s.querySelector('h1,h2,h3')||{}).textContent || ('Slide '+(i+1)),
          notes: resolveNotes(s, i)
        };
      });
...
(function(){
  ...
  /* ===== BroadcastChannel sync + localStorage fallback (for file://) ===== */
  var LS_SYNC_KEY = 'html-ppt-sync:' + location.pathname;
  function lsBroadcast(msg) {
    try { localStorage.setItem(LS_SYNC_KEY, JSON.stringify(Object.assign({}, msg, { _t: Date.now() }))); } catch(e) {}
  }
  function handleRemote(data) {
    if (!data) return;
    if (data.type === 'go') update(data.idx);
    else if (data.type === 'theme' && data.name) {
      currentTheme = data.name;
      [iframeCur, iframeNxt].forEach(function(iframe){
        try {
          iframe.contentWindow.postMessage({ type: 'preview-theme', name: data.name }, '*');
        } catch(err) {}
      });
    }
  }
  ...
  timerCount.textContent = (idx + 1) + ' / ' + total;
    ...
    function cycleTheme(fromRemote){
      ...
      /* Broadcast to other window (audience ↔ presenter) */
      if (!fromRemote) {
        if (bc) bc.postMessage({ type: 'theme', name: name });
        lsBroadcast({ type: 'theme', name: name });
      }
    }
